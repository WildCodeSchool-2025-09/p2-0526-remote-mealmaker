...
function TimerCookModule({ onTimerEnd }: TimerCookModuleProps) {
    const [isRunning, setIsRunning] = useState(false);
    const [isEngaged, setIsEngaged] = useState(false);
    const [remainingTime, setRemainingTime] = useState(0);
    
    // Nouveaux états pour gérer l'édition
    ...
    const handleTimeClick = () => {
        // On empêche la modification si le timer tourne déjà pour éviter les bugs de calcul
        if (isRunning) return; 
        
        setIsEditing(true);
        // On convertit les millisecondes actuelles en secondes pour faciliter la saisie
        setInputValue(Math.floor(remainingTime / 1000).toString());
    };

    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        // Remplace tout ce qui n'est pas un chiffre (\D) par une chaîne vide
    ...
    const handleInputSubmit = () => {
        setIsEditing(false);
        // On reconvertit la valeur saisie (secondes) en millisecondes
        const newTimeMs = Number(inputValue) * 1000;
        setRemainingTime(newTimeMs);
    };
            ...
            <article className="w-auto flex justify-center flex-col gap-2 p-4 border-2 border-primary rounded-2xl bg-background font-bold text-base-content">
                ...
                <div className="text-center h-8 flex items-center justify-center">
                    {isEditing ? (
                        ...
                        <p 
                            className={`cursor-pointer hover:opacity-70 transition-opacity ${isRunning ? "cursor-not-allowed" : ""}`}
                            onClick={handleTimeClick}
                            title={!isRunning ? "Cliquez pour modifier (en secondes)" : ""}
                        >
